...
- インデックス本数を増やし、INSERT/UPDATEが何倍遅くなるかを実測する方法
- これ以上インデックスを増やしてはいけない、と判断するための基準
...
ここからが、参考書だけでは腹落ちしにくい部分です。

「インデックスを貼りすぎると遅くなる」と言われても、読者としては次のことが知りたいはずです。

- どれくらい遅くなるのか
- 何本から危険なのか
- 読み取りが速くなるなら、どこまで書き込み劣化を許容してよいのか
- 使われていないインデックスをいつ消してよいのか
...
しかし、デメリットがあるからインデックスを避ける、という話ではありません。重要なのは、読み取り改善と書き込み劣化を同じテーブルに載せて判断することです。

### 実験：インデックス本数と書き込み劣化を測る

`practice/13_index_write_overhead.sql` では、同じ50,000件のINSERTと一部UPDATEを、追加インデックス0本 / 1本 / 3本 / 6本の状態で実行します。

`id` のPRIMARY KEYインデックスは常に存在するため、この実験では「業務都合で追加したインデックス」が増えたときの差を見ます。

```bash
psql -f ~/udemy-postgres-vol1/chapter02-tuning/practice/13_index_write_overhead.sql
```

出力例は次のような形になります。数値は環境によって変わります。

| 状態 | INSERT劣化率 | UPDATE劣化率 | 見るポイント |
|---|---:|---:|---|
| 追加0本 | 1.00x | 1.00x | 基準値 |
| 追加1本 | 1.1〜1.4x | 1.1〜1.6x | 重要な検索が速くなるなら許容しやすい |
| 追加3本 | 1.5〜2.5x | 2.0〜4.0x | 書き込み頻度が高いテーブルでは要注意 |
| 追加6本 | 2.5x以上 | 4.0x以上 | 「念のため」インデックスを疑う |

この実験で見てほしいのは、絶対値ではなく比率です。Docker DesktopのCPU割り当て、キャッシュ、マシン性能によってミリ秒は変わります。しかし、インデックスが増えるほど書き込み時に更新する構造が増える、という傾向は体験できます。

### これ以上インデックスを増やしてはいけない判断基準

インデックス本数だけで機械的に決めてはいけません。5本でも妥当なテーブルはありますし、2本でも過剰なテーブルはあります。

実務では次の条件が重なったら、追加ではなく整理を検討します。

| 危険信号 | 判断 |
|---|---|
| 追加したインデックスで重要クエリがほとんど速くならない | 追加しない、または削除候補 |
| INSERT/UPDATEが1.5倍以上に悪化し、画面やバッチのSLOに近づく | 読み取り改善と書き込み劣化を比較して再判断 |
| `pg_stat_user_indexes.idx_scan = 0` が1〜2週間続く | 未使用インデックスとして削除候補 |
| 似た複合インデックスが複数ある | 統合できないか検討 |
| インデックスサイズ合計がテーブルサイズに近づく、または超える | ストレージとメンテナンスコストを確認 |
| 低カーディナリティ列に単独インデックスを貼っている | 実行計画で本当に使われるか確認 |

判断の基本は次の式です。

```text
残す価値 = 読み取り改善量 × 使用頻度 × 業務重要度
          - 書き込み劣化 × 書き込み頻度
          - ストレージ/運用コスト
```

**インデックスを作りすぎないこと**。すべてのカラムにインデックスを貼れば速くなるわけではありません。
「実際に遅いクエリ」「頻繁に使われるWHERE条件」「改善効果を測定できたもの」に絞って追加するのが正しいアプローチです。
...
`practice/05_materialized_view.sql` では、あえて新しい注文を1件INSERTします。
その直後に元テーブル `orders` と `mv_daily_sales` を見比べると、元テーブルには存在する売上が、マテリアライズドビューにはまだ出てきません。`REFRESH MATERIALIZED VIEW CONCURRENTLY` を実行して初めて反映されます。

この実験で、「速いけど古い」という性質を体で覚えます。
...
ただし、CTEを使えば必ず速くなるわけではありません。
`practice/06_cte.sql` では、あえて `orders` 全件を `MATERIALIZED` なCTEに固定してから絞り込む悪い例を入れています。先に巨大な中間結果を作ってしまうと、分割しているのに遅くなることがあります。

見るべきポイントは「WITH句を使ったか」ではなく、「最初の段階で行数を減らせているか」です。
...
`practice/09_query_patterns.sql` では、B-treeインデックスを作ったうえで `LIKE '%12345%'` を実行します。
それでも基本的にB-treeは効かず、Seq Scanになりやすいことを確認します。その後、`pg_trgm` + GINインデックスを作成し、同じ部分一致検索で `Bitmap Index Scan` に変わることを確認します。

ここでは「インデックスがあるか」ではなく、「その検索条件に合う種類のインデックスか」を見ることが重要です。
...
`practice/10_statistics.sql` では、`orders_2025` という新しいテーブルに大量データを入れ、ANALYZE前後で `rows=` と `actual rows=` を比較します。
実行時間だけでなく、「プランナーの予測がどれくらい外れているか」を見るのがポイントです。

統計情報のズレは、クエリそのものが悪くなくても実行計画を悪化させます。大量INSERTやバッチ後に `ANALYZE` が必要になる理由を、ここで体験します。
...
6. インデックスは書き込み負荷を増やすため、INSERT/UPDATEの劣化率も測定して判断する
7. インデックス追加の判断は、本数ではなく「読み取り改善量」「書き込み劣化」「使用頻度」「業務重要度」で決める
8. マテリアライズドビューで重い集計クエリをキャッシュして高速化できる
9. WITH句（CTE）で「先に絞り込んでから結合する」構造にすることで多テーブルJOINを高速化できる
10. 型ミスマッチ・関数ラップ・先頭列スキップの3パターンがインデックスを無効化する
11. `pg_stat_user_indexes` で使われていないインデックスを発見して削除できる
12. N+1問題は `pg_stat_statements` の `calls` 異常値で発見できる
13. `IN（サブクエリ）` は大量データでスケールしない → `EXISTS` または `JOIN` に書き換える
14. `LIKE '%キーワード%'` は `pg_trgm` + GINインデックスで解決できる
15. 統計情報のズレが実行計画の誤判断を引き起こす → `ANALYZE` で解消する
16. OFFSET はページが深くなるほど遅くなる → 大量データには WHERE + ソートキーのキーセット方式を使う

**パフォーマンスチューニングの鉄則**:

> 計測せずに推測で動いてはいけない。
> `EXPLAIN ANALYZE` で証拠を確認してから、インデックスを追加する。
> インデックスは「貼れば速くなる」ではない。読み取り改善だけでなく、書き込み劣化率と使用頻度まで測って設計する。
