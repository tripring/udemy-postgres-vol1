...
「クエリが遅い」「レポート作成が毎月つらい」「同時に注文が入って在庫がマイナスになった」。
この講座では、そうした現場のトラブルをいきなり機能名で解決しようとはしません。

まず、UdeMartのメンバーが困っている場面を見ます。次に、岡野くんが何を観察し、どのSQLを書き、どこで設計判断をしたのかを追います。最後に、同じ考え方を自分の業務に持ち帰れるように、PostgreSQLの機能として整理します。

---

## ストーリーで学ぶ流れ

この講座では、UdeMartで起きる小さな事件をきっかけにして、必要なPostgreSQLの機能を順番に学びます。単に構文を暗記するのではなく、「この場面なら、なぜこのSQLや設計が必要なのか」を追いながら進めます。

各章は、次の流れを意識して構成しています。

1. 業務上の困りごとを確認する
2. まず計測・観察して、原因の候補を絞る
3. PostgreSQLの機能を使って改善する
4. 改善後の副作用や運用上の注意点を確認する

さらに、この講座では「悪化を体験する」ことを重視します。

参考書には「インデックスを貼りすぎると遅くなる」「ロック待ちに注意」「OFFSETは深いページで遅い」と書かれています。しかし、実務で必要なのは、どれくらい遅くなるのか、どの時点で危険信号と判断するのか、どう戻すのかを自分で説明できることです。

そのため、各章ではあえて悪い状態を作ります。インデックスを増やしすぎてINSERT/UPDATEを遅くする、ロック待ちを発生させる、深いOFFSETを実行する、パーティションプルーニングが効かない条件を書く。その上で、計測結果を見ながら、なぜこの講座で扱う対応策が必要なのかを理解していきます。

| 章 | UdeMartで起きること | 学ぶこと |
|---|---|---|
| 01 | マーケティング担当の皆川くんが、月次売上レポートを毎月Excelで手作業している | ウィンドウ関数、ランキング、前月比、JSONB |
| 02 | 受注管理画面や経営ダッシュボードが遅くなり、現場から苦情が届く | `pg_stat_activity`、`pg_stat_statements`、実行計画、インデックス、書き込み劣化測定、マテリアライズドビュー |
| 03 | フラッシュセールで残り1個の商品が二重購入され、在庫がマイナスになる | トランザクション、ロック待ち、デッドロック、楽観ロック |
| 04 | 注文テーブルが肥大化し、月次レポートが終わらなくなる | パーティショニング、プルーニング失敗、パーティション運用 |
...
SSH接続後、bastion 内で実行します。ホストPCにクローンした教材ディレクトリは、bastion 内でも同じ名前の `~/udemy-postgres-vol1` として見えます。
...
psql -f ~/udemy-postgres-vol1/chapter02-tuning/practice/01_pg_stat_activity.sql
```
