...
ここで岡野くんが考えたのは、「Excel作業をSQLに置き換える」ことだけではない。
毎月同じ手順を人が繰り返しているなら、そこには必ずミスの余地がある。貼り付ける範囲を1行ずらす、前月のシートを参照したままにする、ランキングの同点処理を月ごとに変えてしまう。SQLとして定義できれば、作業時間だけでなく、判断基準も安定する。

このチャプターでは、皆川くんの手作業を少しずつSQLに移していく。最初はランキング、次に累計、前月比、移動平均。最後に「商品カテゴリごとに属性が違う」という別の悩みを JSONB で扱う。

この章でも、良いSQLだけを見て終わりにはしない。

- `GROUP BY` だけで明細レポートを作ろうとして、元の行が消えて困る
- JSONBを便利だからと何でも入れて、検索や制約がつらくなる
- GINインデックスで検索は速くなるが、容量と書き込みコストが増える
- アプリ側の「存在チェックしてからINSERT」で競合が起きる

こうした悪化パターンを見たうえで、ウィンドウ関数、JSONBの使いどころ、UPSERTの必要性を理解していく。
...
皆川くんのレポートでは、注文1件ごとの明細を残したまま、「その顧客の累計購入金額」や「カテゴリ内の順位」を横に添えたい場面が多い。`GROUP BY` で集計してしまうと、元の注文行は消える。あとから「この順位の商品はどの注文で売れたのか」を見ようとしても、もう行の粒度が変わっている。

一方、ウィンドウ関数は元の行を残したまま、別の列として集計結果を付けられる。レポート作成で「明細も見たい、集計値も見たい」という場面に強い。
...
JSONBは万能の逃げ道ではありません。商品属性のようにカテゴリごとに項目が変わるデータには向いていますが、注文日時、顧客ID、ステータスのように検索・JOIN・制約で頻繁に使う列までJSONBに入れると、SQLは読みにくくなり、制約も弱くなり、インデックス設計も難しくなります。

この講座では、JSONBを「スキーマ設計を放棄するための箱」ではなく、「変化しやすい属性だけを逃がすための選択肢」として扱います。
...
ここでも、速くなる面だけを見ないことが重要です。GINインデックスはJSONB検索には強力ですが、B-treeよりサイズが大きくなりやすく、INSERT/UPDATE時の更新コストも増えます。検索頻度が低い属性にまで「念のため」GINインデックスを貼ると、Chapter 02で体験するインデックス過多と同じ問題が起きます。
...
月次レポートが安定してくると、今度は外部システムとの連携が増えてくる。倉庫システムから在庫数が届く、広告システムから商品別の成果が届く、夜間バッチで集計値を反映する。こういう処理では、「商品IDがあれば在庫数を更新、なければレコードを作る」という分岐が頻繁に発生する。
...
この方法は2セッションが同時に同じキーを処理したとき、どちらも「存在しない」と判断して INSERT を試みてしまう（重複キーエラー）。岡野くんはこの種類の処理を、アプリの if 文ではなくデータベースの制約と1つのSQLに寄せることにした。

`INSERT ... ON CONFLICT` は「存在すれば更新、なければ挿入」を1文で**原子的に**解決する。
