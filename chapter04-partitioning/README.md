...
この章でも、良い状態だけを見て終わりにはしない。

- 通常テーブルで月次レポートを実行し、全件スキャンの重さを確認する
- パーティションを作っても、WHERE句にパーティションキーがなければ速くならないことを確認する
- 関数で `ordered_at` を包むと、プルーニングが効きにくくなることを確認する
- DEFAULTパーティションにデータが流れると、運用ミスに気づきにくくなることを確認する
- 古いデータ削除をDELETEで行う場合と、パーティションDETACHで行う場合の考え方を比較する

パーティショニングは「大きいテーブルなら入れる」機能ではありません。効く条件、効かない条件、運用ミスで悪化する条件を体験してから導入判断できるようにします。
...
```bash
psql -f ~/udemy-postgres-vol1/chapter04-partitioning/setup.sql
```
...
高橋くんの月次レポートが遅くなった原因は、SQLの書き方だけではなかった。
`orders` テーブルそのものが、過去3年分の注文をすべて抱え込む巨大な箱になっていた。今月分を見たいだけでも、DBは古い注文も含めた大きなテーブルを相手にしなければならない。

岡野くんは、注文データを「月ごとの引き出し」に分けることを考えた。アプリケーションからは今まで通り `orders` という1つのテーブルに見える。しかし内部では、2024年1月、2024年2月、2024年3月という単位で物理的に分かれている。この仕組みがパーティショニングです。
...
逆に、WHERE句に `ordered_at` の範囲条件がなければ、PostgreSQLはどのパーティションを読めばよいか判断できません。パーティショニングを導入しても、クエリがパーティションキーを使わなければ効果は出ません。

### いつパーティショニングを使うべきか

パーティショニングは万能ではなく、導入コストもあります。次のような条件が重なったときに検討しましょう。

岡野くんも、最初からパーティショニングを入れたわけではありません。注文が数万件のころは、普通のテーブルとインデックスで十分でした。月次レポートが遅くなり、古い注文のアーカイブも必要になり、運用上の困りごとが重なってきたから導入を判断しました。
...
`practice/05_partition_operations.sql` では、あえて2025年2月のパーティションを作らないまま、2025年2月の注文をINSERTします。
エラーにはならず、`orders_default` に入ってしまうことを確認します。

これは本番でよくある運用事故です。「注文は入っているので画面上は動いている」が、後から見ると想定外のパーティションにデータが溜まっていた、という状態になります。DEFAULTパーティションを作るなら、そこにデータが入っていないかを監視する必要があります。
...
本番でいきなり既存の `orders` を消して作り直すことはできません。アプリケーションは動いており、注文も入り続けます。

この教材では学習用にシンプルな手順で移行しますが、考え方としては「新しい器を作る」「既存データを移す」「件数と期間を確認する」「切り替える」という順番です。実務ではメンテナンス時間、書き込み停止、差分同期、ロールバック手順まで含めて計画します。
...
実行計画を見て、スキャン対象の子パーティションを確認します。

```
Seq Scan on orders_2024_01
```

36パーティションのうち `orders_2024_01` だけが読まれていれば、スキャン対象が 1/36 になっていることが確認できます。これがパーティションプルーニングです。
...
パーティショニングは作って終わりではありません。
月単位で分けるなら、来月のパーティションを事前に作る必要があります。古くなったデータを削除するなら、DELETEで何百万行を消すのではなく、古いパーティションを切り離すほうが速く安全です。

高橋くんのレポートを速くするだけでなく、DB運用そのものを軽くする。ここがパーティショニングの大きな価値です。
...
ただし、DEFAULTパーティションがあるとエラーにならない分、作成漏れに気づきにくくなります。月次運用では「翌月パーティションを作る」だけでなく、「DEFAULTにデータが入っていないことを確認する」チェックも必要です。
...
この章の実習では、同じ2022年1月分に対して、まず `DELETE` をトランザクション内で実行して重さを確認し、`ROLLBACK` します。その後、`DETACH PARTITION` で切り離します。

大量DELETEは1行ずつ削除し、WALも発生し、インデックス更新も必要です。一方、DETACHはパーティションを親テーブルから切り離すメタデータ操作です。古い月単位でデータを消したいなら、DELETEではなくDETACHやDROPを検討する理由を体験できます。
...
| `EXPLAIN ANALYZE` でプルーニング確認 | 読まれた子パーティション数で効果を確認 |
